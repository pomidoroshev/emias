...
def gen_id():
    return ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(21))
...
def get_schedule(doctor_name):
    response = requests.post(
        url=BASE_URL, params={"getAvailableResourceScheduleInfo": "null"},
        json={
            "jsonrpc": "2.0",
            "id": gen_id(),
...
def run():
    for doctor_name in config['doctors']:
        ...
        if slot and slot - timedelta(days=config['catch_within_days']) <= datetime.now():
